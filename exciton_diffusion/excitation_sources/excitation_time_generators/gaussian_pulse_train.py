...
from exciton_diffusion.excitation_sources.excitation_time_generators.base import (
...
class GaussianPulseTrainGenerator(ExcitationTimeGenerator):
    ...
    pulse_full_width_half_max_s: float
    ...
    def generate(
        ...
        delays_from_pulse_peaks_s = np.random.normal(
            scale=self.pulse_standard_deviation_s, size=total_excitations_generated
        )
        excitation_pulse_indices = np.random.randint(
            low=min_pulse_index, high=max_pulse_index, size=total_excitations_generated
        )

        excitation_times = (
            excitation_pulse_indices * self.repetition_period_s
        ) + delays_from_pulse_peaks_s
